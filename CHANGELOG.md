...
- **在线小说阅读器**：搜索小说→点章节直接阅读；字号调节（A−/A+）、上一章/下一章、点击正文切换工具栏、切章回顶、加载/错误/空三态
- **聚合小说/聚合漫画分类**：书源选择弹窗分区展示，Z-Library 与 Legado 文字源归入「聚合小说」，聚合搜索按类别互斥过滤
- **小说/漫画标签**：卡片与详情页按源类型显示正确标签，书源管理页小说源带徽章
- **聚合结果可折叠**：每个源分组标题栏折叠键，单独收起/展开，换关键词自动重置

### 🔧 Bug 修复

- **「暂无可用章节」**：支持 `ruleBookInfo.tocUrl` 两步目录解析（详情页→目录页），修复就爱文学（92xs.info）等目录独立页源
- **POST 搜索书源导入失败**：不再跳过含 `postUrl` 的源，请求体模板透传，JSON/form 自动判 Content-Type
- **CSS 混合 Legado 索引**：`a.0` 写法选择失败时剥离 `.N` 后缀重试
- **Z-Library 节点不可用**：默认域与内置节点切换为国内可直连镜像，旧域名垫底

### 🎨 UI 与体验（合入主干）

- 四 Tab 页滚动联动折叠头部（密度无关阈值 + 模式切换回顶）
- 聚合搜索「源组速跳」面板：组头即入口，弹性滚动直达
- JS 漫画源错误可见化 + picacg 被墙源自动代理路由；纯数字 ID 搜索无结果时直达详情

### 🔍 可观测性

- 新增 SourceLog 调试日志（内存环形缓冲 300 条），书源管理页可查看/复制/清空
- 网络错误透传真实 HTTP 状态（NetworkDetail），不再统一报「无法连接网络」
