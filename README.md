...
| APK 体积                 | 约 21.5MB（含 ONNX Runtime；OCR / 气泡模型按需下载）                               |

**核心能力速览**

| 方向        | 能力                                                                              |
| --------- | ------------------------------------------------------------------------------- |
| 阅读        | 五种文字翻页动画、串珠快速翻页、真实排版分页引擎、漫画五模式 + 三引擎（含 OpenGL 仿真卷页）                            |
| 找书        | Z-Library / MangaDex / Venera JS 源 / Legado JSON 源 / ehentai，多源并发聚合搜索，逐源即时出结果          |
| 下载        | WorkManager 后台队列、3 路并发、断点续传、真实格式校验、失败重试                                          |
| 生产内容      | 漫画整页翻译：本地 OCR + 气泡分割 + 双引擎翻译（自定义 AI / 在线），译文按气泡形状渲染并锚定原文位置                       |
| 打磨        | 苹果感设计体系、统一动效令牌与五级触觉、「我喜欢的」收藏体系、跨品牌一致性适配                                         |
...
- **串珠快速翻页**：长按页面任意位置 1s 唤出——所有页面整页刚体贴在圆柱内壁上环绕圆心排列（等半径、仅 Y 轴单次旋转、无缩放，侧页变窄纯靠透视收窄，边缘完整连续无拼条痕迹）；拖动沿弧 1:1 跟手，松手磁吸最近页点击进入；甩动按力度惯性连翻（普通快甩约 3 页、极速甩封顶 12 页）；进场 / 逐页掠过 / 提交 / 取消四处伴随真实翻书声（page_flip.ogg）
...
**书源管理页**（v1.1.0 重排版）：三张玻璃卡 + 一个虚线导入行（快捷入口 / 小说 / 漫画），分组数量并入卡片头部；行内为头像 + 名称 + ID（等宽）+ 「需登录」标签 + 登录 / 节点胶囊 + 开关；除大标题外全页无裸文字，壁纸经模糊与遮罩后只作氛围色。导入收敛为一条入口，支持本地文件 / 粘贴 JSON / 网络导入三种方式。
...
- **跨品牌一致性**（不因机型与系统字体设置而走样）：全局 `fontScale` 钳制到 `[0.85, 1.15]`（443 处硬编码字号一次性获救）；`LocalAppBottomInset` 作为底部安全区单一事实来源（三键导航机型不再被压栏）；`DesignTokens` 与 `AdaptiveSpec` 断点体系落地到页面外边距与书架列数；补齐 `configChanges`（折叠屏开合不再重建 Activity 丢进度）

- **苹果感设计体系**（系统性改造，非逐页贴皮）：SF Pro 规格的排版层级（字号越大字距越紧）、iOS 弹簧规格驱动的弹窗 / 底部面板 / 主题色过渡、iOS 式整体按压反馈（替代叠白矩形，深色卡不再闪白斑）、关闭 Android 滚动边缘发光改用回弹、进入阅读页的 iOS push 转场
...
- **第二套玻璃 GlassKit**（v1.1.0，仅书源管理页与搜索历史卡）：基于 backdrop 库的实时图层录制 + GPU 透镜折射，形状为连续曲率圆角，着色只用中性白 / 中性深；卡片在显隐动画期间自动降为轻量档，静止后恢复完整折射。全 App 既有的玻璃组件保持稳定，两套互不影响
...
- **统一动效令牌 `AppMotion`**：弹簧 stiffness ≈ 300 / damping ≈ 22（拿起与落下 damping ≈ 16，更「重」一些）；普通过渡 200–250ms easeOutCubic；粒子与飞入总时长 ≤ 700ms——全 App 一处调参、处处一致

- **五级语义触觉**：selection / light / medium / heavy / success，直接驱动系统 Vibrator（Compose 内置 HapticFeedbackType 做不出分级）；受设置开关控制，无马达 / 无权限时静默降级

- **按压与惯性反馈**：按钮 / 列表行 / 胶囊统一按压反馈与缩放进退；玻璃卡片随按压跷跷板倾斜、随滚动手势惯性摆动并极软回正；固定光源的 AGSL 法线光照压痕（Android 13+，低版本自动降级渐变模拟），整套 UI 有真实「玻璃厚度」

- **列表动效完整**：目录 / 搜索 / 书签 / 漫画目录 / 书架瀑布流均补齐重排动画与元素 key（此前有动画却没 key，重排会错位）

- **键盘与滚动的可感流畅**（v1.1.0）：输入法避让改用 IME 动画目标值（不再逐帧挤压内容），折叠动画的状态读取下沉到组件内部（重组范围从整页收缩到局部），输入法动画期间不做卡片位移动画

- **无障碍与省电适配**：读取系统「减少动态效果」逐项降级为静态（信息不丢）；尊重系统「降低透明度 / 高对比度」，玻璃改近乎不透明底并去掉折射

#### 「我喜欢的」收藏体系

- **一键收藏**：书架与详情页的爱心按钮，含填充动画与状态语义（「喜欢，加入我喜欢的」/「已喜欢，点击取消喜欢」）；无可迁移来源时给出明确禁用提示

- **视觉标识**：书架卡片右下角品牌红角标（#FF4D6D，与爱心中部同色），一眼可见哪些是收藏

- **书架分段**：「我的书架 / 我喜欢的」分段切换（滑动指示器），收藏从书架列表中独立成区，不与分类归档混在一起

- **换源不丢数据**：更换书源时列出候选来源，确认后按章节顺序迁移收藏、阅读进度与已读状态

- **空状态友好**：没有收藏时给出引导文案，而不是一片空白

- **数据落库**：收藏独立 Room 实体与 DAO，与书籍数据分离，卸载重装前可用 JSON 导出（Roadmap）
...
3. 想用自己的规则：书源管理 → 导入入口 →「粘贴 JSON 配置」或「从本地文件导入」，格式兼容 Legado（`@css:` / `@json:` 语法）；书库 → 帮助手册内有 JSON 模板。
...
书架顶部切到「我喜欢的」分段，或直接点书籍的爱心按钮收藏。收藏后书架卡片会出现红色角标；换书源时收藏、阅读进度与已读状态会一并迁移。
...
└── ui/
    ...
    ├── glasskit/                # 第二套玻璃（实时录制 + 透镜折射，v1.1.0）
    ├── favorite/                # 爱心按钮 / 「我喜欢的」面板 / 换源迁移
    ├── shelf/                   # 书架交互（多选、胶囊、选择宿主）
    ├── feedback/                # 动效与触觉统一令牌（AppMotion）
    ├── mascot/                  # Roxy 吉祥物动画
    ├── adaptive/                # 统一宽度断点规范
...
- 动效与触觉统一走 `ui/feedback/AppMotion`，玻璃组件分两套：全 App 通用的 `ui/components` 与书源 / 搜索历史专用的 `ui/glasskit`
...
**Q5：模拟器上开翻译闪退？**
Release 包仅含 arm64 ONNX 库，x86\_64 模拟器经 ARM 转译运行 ONNX 会 SIGSEGV，属模拟器限制；Debug 包附带 x86\_64 库可正常使用（构建时加 `-PincludeX86`）。

**Q6：自定义 JSON 书源怎么写？**
书库 → 帮助手册 →「查看 JSON 模板」；规则兼容 Legado 的 `@css:` / `@json:` 语法。

**Q7：收藏（我喜欢的）会丢吗？**
收藏单独落库，换书源时可通过「换源迁移」按章节顺序带走收藏、进度与已读状态。数据存于应用私有目录，卸载即清除。
...
- 动效与触觉改动请使用 `ui/feedback/AppMotion` 的既有令牌，不要在页面里散写数值
...
| 版本    | 日期         | 主题                                                       | 要点                                                                                          |
| ----- | ---------- | -------------------------------------------------------- | ------------------------------------------------------------------------------------------- |
| 1.1.0 | 2026-09-24 | 书源管理重排版 · 第二套玻璃 · 搜索键盘卡顿修复                               | 书源页三卡一行的排版与可读性；GlassKit（实时录制 + 透镜折射）；IME 逐帧挤压与整页重组两个卡顿根因                                          |
| 1.0.7 | 2026-09-21 | 前端深度审查 · 苹果感设计体系                                         | 跨品牌适配（字体钳制 / 安全区单一来源 / 设计令牌）、年视图日历重写、数据统计与隐私模式修复、动效与转场补齐、APK 瘦身 23MB → 19.7MB                |
| 1.0.6 | 2026-09-08 | 串珠快速翻页（整页刚体）                                             | 长按 1s 唤出圆柱书架式快速翻页，甩动惯性连翻，真实翻书声；漫画加载单圈、下载型小说源直连下载                                            |
| 1.0.5 | 2026-09-05 | 漫画阅读器整体重做 · 漫画整页翻译                                       | 28 条修复升级 + 四路终审 + 六条实测根治；CURL 双页书脊 / 刚体封面 / 透纸背面；本地 OCR + 气泡分割 + 双引擎翻译；Z-Library 节点容灾与 PoW 自动求解 |
| 1.0.1 | 2026-08-29 | 在线阅读与聚合增强                                                | 在线小说阅读器、漫画 / 小说分类聚合、Legado 兼容增强（POST 搜索 / tocUrl / 调试日志）、每源 6 条预览 + 展开全部                   |
| 1.0.0 | 2026-08-28 | 首个正式版                                                    | 本地书籍与漫画阅读、书源聚合搜索与下载、阅读统计、LiquidGlass UI、MOBI/AZW3 自研解析、仿真卷页、场景系统、Anime4K 增强                 |
