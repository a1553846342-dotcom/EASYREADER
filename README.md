...
和同类软件有什么不同？**它不只是阅读器，更是一台资源下载器**：Z-Library 深度原生接入（自动过 DiamWall 验证、节点管理、账号登录、真实下载链接解析），搜索到的书不是“只能看看”，而是直接下载到书架离线阅读；漫画支持单章/批量下载，下载中心常驻后台，断点续传、失败重试、文件校验一条龙。再加上 **Venera JS 漫画源生态**（拷贝漫画、comick、漫蛙吧、goDa 等开箱即用）、**自定义 JSON 书源**（兼容 Legado 规则）和**一整套液态玻璃设计语言**（悬浮收缩 Tab 栏、毛玻璃弹窗、果冻开关、Juno 滑块、凝胶按钮）。别人只做一个点，我们做了一整套。
...
| 一句话简介 | 支持本地 TXT/EPUB/漫画导入、自定义书源、在线书库聚合搜索与下载的安卓阅读器兼资源下载器 |
...
- **Z-Library 全链路下载**：搜索 → 详情 → 自动过 DiamWall 验证 → 解析真实下载链接 → 断点续传 → 文件校验 → 自动入库书架，全程无需浏览器。
...
- **防假文件校验**：下载完成自动检测“HTML 错误页伪装文件”，EPUB 校验 ZIP 结构，坏文件直接标记失败可重试。
- **毛玻璃下载卡片**：实时显示封面、进度、速度、剩余大小；下载中心常驻，随时回来查看。
